...
class PoeLoginSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        # we pass `email` as the `username` value. Remember that, in our User
        # model, we set `USERNAME_FIELD` as `email`.
        # user = User.objects.get(email='namlehong@gmail.com')
...
class UserSerializer(serializers.ModelSerializer):
    ...
    avatar = serializers.CharField(source='profile.image', read_only=True)

    # extra for poe profile
    settings = serializers.JSONField(source='profile.settings', read_only=True)
    poe = serializers.JSONField(source='profile.poe_account', read_only=True)
    approved = serializers.IntegerField(source='profile.positive_karma', read_only=True)
    disapproved = serializers.IntegerField(source='profile.negative_karma', read_only=True)
    ...
    class Meta:
        ...
        fields = (
            ...
            'image', 'role', 'name', 'avatar', 'settings', 'poe', 'session_id', 'approved', 'disapproved', 'id'
        )
